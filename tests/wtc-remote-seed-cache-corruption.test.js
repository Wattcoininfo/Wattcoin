...
async function run() {
  ...
  try {
    ...
    assert.strictEqual(thrownInLoad, null, 'loadCachedRemoteSeedPeers() must not throw on a corrupt cache file');
    const cached3 = mgr3.loadCachedRemoteSeedPeers();
    assert.deepStrictEqual(cached3, [], 'corrupt cache should yield [] from loadCachedRemoteSeedPeers');
