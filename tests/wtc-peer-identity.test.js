...
async function run() {
  ...
  try {
    ...
    assert.strictEqual(nodeA.getPrimaryAddress(), nodeB.getPrimaryAddress(), 'wallet clone should keep the same mining address');
    assert.notStrictEqual(nodeA.getPeerIdentity(), nodeB.getPeerIdentity(), 'peer identities should remain per-install');
    assert.strictEqual(nodeA.handleGetTip().peerIdentity, 'a'.repeat(64), 'tip response should expose node A peer identity');
    assert.strictEqual(nodeB.handleGetTip().peerIdentity, 'b'.repeat(64), 'tip response should expose node B peer identity');
    ...
    assert.strictEqual(readiness.connections, 1, 'matching peer identity on a non-self URL should still count as a reachable peer');
    ...
    assert.strictEqual(selfReadiness.connections, 0, 'actual self URLs must still be skipped even when the peer identity matches');
    ...
    assert.strictEqual(tunnelOnlyReadiness.connections, 1, 'live connected tunnel peers should count toward readiness even without HTTP peer targets');
    assert.strictEqual(tunnelOnlyReadiness.spendReady, true, 'a live connected tunnel peer at network height should keep wallet readiness spendable');
    ...
    assert.strictEqual(directoryFallbackReadiness.connections, 1, 'reachable directory peers should count even when active peers exist');
    assert.strictEqual(directoryFallbackReadiness.bestPeerHeight, 6, 'reachable directory peer height should feed readiness');
    assert.strictEqual(directoryFallbackReadiness.spendReady, false, 'higher directory peer height should still keep readiness syncing');
    assert.strictEqual(directoryFallbackReadiness.status, 'syncing', 'reachable higher directory peer should surface syncing readiness');
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      assert.strictEqual(failedReadiness.connections, 0, 'temporarily failing peer should not count as reachable before recovery');
    }
    const recoveredReadiness = await recoveredPeerNode.getWalletReadiness();
    assert.strictEqual(recoveredReadiness.connections, 1, 'readiness should retry a recovered peer even after backoff was entered');
    assert.strictEqual(recoveredReadiness.spendReady, true, 'recovered peer at local height should restore spend-ready readiness');
  } finally {
    rmrf(baseDir);
  }
