...
function testMempoolPressureHighAlertWhenAboveThreshold() {
  ...
  assert.ok(response.activeAlerts.find(a => a.code === 'mempool.pressure.high'), 'alert should fire above threshold');
}
...
function testMultipleActiveAlertsAllAppearInResponse() {
  const snapshot = {
    timestamp: '2026-04-21T00:00:00.000Z',
    chain: { localHeight: 100, nodeLagBlocks: 0, latestBlockAgeMs: 21 * 60_000, forkRatePerHour: 0, rollbackMedianDepth: 0 },
...
function testStatusIsDegradedForWarnOnlyAlerts() {
  ...
  assert.ok(response.activeAlerts.every(a => a.severity === 'warn'), 'all active alerts should be warn level');
}
