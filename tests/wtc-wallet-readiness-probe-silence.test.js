...
async function run() {
  ...
  try {
    ...
    try {
      ...
      assert.strictEqual(readiness.ok, true, 'getWalletReadiness() should not throw when all peer probes fail');

      // Connections = 0 because every probe threw.
      assert.strictEqual(readiness.connections, 0, 'connections should be 0 when all peer probes throw');

      // spendReady requires connections > 0, so it must be false.
      assert.strictEqual(readiness.spendReady, false, 'spendReady should be false when connections = 0');
      ...
      const relevantWarnings = [...warnMessages, ...errorMessages].filter(
        (m) =>
