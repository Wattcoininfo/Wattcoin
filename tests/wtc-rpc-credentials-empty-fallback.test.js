...
function run() {
  ...
  try {
    ...
    assert.strictEqual(config1.rpcUser,     'cfg-user',
      'rpcUser should come from WATTCOIN_RUNTIME_CONFIG when env var is absent');
    assert.strictEqual(config1.rpcPassword, 'cfg-pass',
    ...
    assert.strictEqual(config2.rpcUser,     'env-user',
      'env-var WATTCOIN_RPC_USER should override config-file rpcUser');
    assert.strictEqual(config2.rpcPassword, 'env-pass',
    ...
    assert.strictEqual(typeof config3.rpcUser,     'string',
      'rpcUser must be a string even when credentials are empty');
    assert.strictEqual(typeof config3.rpcPassword, 'string',
      'rpcPassword must be a string even when credentials are empty');
    assert.strictEqual(config3.rpcUser,     '',
      'RISK DOCUMENTED: rpcUser is empty string when config provides no value — ' +
      'depends on wattcoind to reject empty auth');
    assert.strictEqual(config3.rpcPassword, '',
      'RISK DOCUMENTED: rpcPassword is empty string when config provides no value');

    console.log('[PASS] rpc-credentials: config-file and env-var override paths verified; empty-string fallback documented');
  } finally {
