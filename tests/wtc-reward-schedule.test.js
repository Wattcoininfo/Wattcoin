...
// Tier 1, first block
assert.strictEqual(energyForHeight(1), BLOCK_ENERGY_WH, 'Height 1 (first mined block) should require exactly 10 MWh');
assert.strictEqual(rewardForHeight(1), 500, 'Height 1 should pay 500 WTC (Tier 1 reward)');

// Tier 1, last block (height 2000)
assert.strictEqual(
  energyForHeight(2000),
  BLOCK_ENERGY_WH,
  'Height 2000 (last Tier-1 block) should still require exactly 10 MWh',
);
assert.strictEqual(rewardForHeight(2000), 500, 'Height 2000 should still pay 500 WTC (still Tier 1)');
...
assert.strictEqual(tier1Total, SUPPLY_PER_TIER, 'Tier 1 (heights 1–2000) must produce exactly 1,000,000 WTC total');
...
for (let tier = 1; tier <= MAX_MINING_TIERS; tier++) {
  ...
  assert.strictEqual(
    energyForHeight(startHeight),
    BLOCK_ENERGY_WH,
    `Tier ${tier} (first block at height ${startHeight}) should require exactly 10 MWh`,
  );
  assert.strictEqual(rewardForHeight(startHeight), reward, `Tier ${tier} first block should pay ${reward} WTC/block`);
  // Last block of this tier
  assert.strictEqual(
    energyForHeight(startHeight + blocksThisTier - 1),
    BLOCK_ENERGY_WH,
...
assert.strictEqual(
  Math.round(totalMined),
  HARD_CAP - GENESIS_PREMINE,
...
assert.strictEqual(rewardForHeight(startHeight), 0, 'After all tiers exhausted, reward should be 0');
assert.strictEqual(energyForHeight(startHeight), 0, 'After all tiers exhausted, energy requirement should be 0');
...
console.log(
  `  Total mined supply: ${Math.round(totalMined).toLocaleString()} WTC + ${GENESIS_PREMINE.toLocaleString()} genesis = ${HARD_CAP.toLocaleString()} WTC hard cap.`,
);
