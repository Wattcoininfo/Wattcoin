...
function makeNode(id, dir) {
  return createWtcNode({
    ...
    getActivePeers:  () => [],
    requestPeerJson: async () => { throw new Error('no peers in counterfeit test'); },
  });
}
...
await test('Attack 1 — forged signature is rejected', async () => {
  ...
  try {
    ...
    const sigInput = JSON.stringify({ id: fakeTx.id, from: fakeTx.from, to: fakeTx.to, amount: fakeTx.amount, fee: fakeTx.fee, nonce: fakeTx.nonce });
...
await test('Attack 2 — inflated block reward is rejected', async () => {
  ...
  try {
    ...
    assert.match(String(result.reason), /rewardAddresses sum|reward mismatch/i,
...
await test('Attack 4 — reward redirected to valid attacker address is allowed (sum matches)', async () => {
  ...
  try {
    ...
    const block = await node.mineBlock(node.getPrimaryAddress(), {
      energyWh: TIER1_ENERGY,
...
await test('Attack 5 — sub-threshold energy block is rejected', async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => node.mineBlock(node.getPrimaryAddress(), {
        energyWh: TIER1_ENERGY - 1,
        proofCommitment: 'low-energy-attack',
      }).then(r => { if (r && r.ok === false) throw new Error(r.reason); }),
