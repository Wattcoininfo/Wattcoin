...
conn.on('ready', () => {
  conn.sftp(async (err, sftp) => {
    ...
    for (const dir of ['htdocs', 'htdocs/releases', 'htdocs/assets']) {
      console.log(`\n=== ${dir} ===`);
      await new Promise((resolve) => {
        sftp.readdir(dir, (e, list) => {
          ...
          for (const f of list) {
            const type = f.longname.startsWith('d') ? '[DIR]' : '     ';
            const sizeMB = (f.attrs.size / 1024 / 1024).toFixed(2);
            console.log(`  ${type} ${f.filename}  (${sizeMB} MB)`);
          }
          resolve();
        });
      });
...
conn.connect({
  ...
  hostHash: 'sha256',
  hostVerifier: (hash) => normalizeFingerprint(hash) === expected,
  tryKeyboard: true,
