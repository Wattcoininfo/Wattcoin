...
Export-PfxCertificate -Cert $leafCert -FilePath $signPfx -Password $securePassword | Out-Null
...
# --------------------------------------------------------------------------- #
#  Save password for auto-signing during builds
# --------------------------------------------------------------------------- #
Set-Content -Path (Join-Path $OutputDir ".password") -Value $PfxPassword -NoNewline
Write-Host "  [OK] Password saved to: $(Join-Path $OutputDir '.password')" -ForegroundColor Green
