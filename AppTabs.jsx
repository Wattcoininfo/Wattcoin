...
function loadPersistedSentTransactions() {
  try {
    ...
    const parsed = JSON.parse(raw);
    if (!Array.isArray(parsed)) return [];
    return parsed.filter((entry) => entry && typeof entry === 'object').map((entry) => ({
...
export default function AppTabs() {
  ...
  useEffect(() => {
    ...
    hw.invoke('wattcoin-get-probe-log').then(res => {
  ...
  useEffect(() => {
    ...
    return () => { if (probeLogSaveTimer.current) clearTimeout(probeLogSaveTimer.current); };
  }, [probeLog]);
  ...
  const handleSelectedWalletAddressChange = useCallback(async (address) => {
  ...
  const protectedTabLocked = MINER_PROTECTED_TABS.has(activeTab) && minerAccessPolicy.passwordRequired && !minerUnlocked;
  ...
  const selectedAddressKey = (typeof selectedWalletAddress === 'string' ? selectedWalletAddress.trim() : '') || '__legacy__';
  const energy = Math.max(0, Number(energyByAddress[selectedAddressKey]) || 0);
  const setEnergy = useCallback((updater) => {
  ...
  const syncBalancesFromLedger = useCallback(async () => {
    ...
    try {
      ...
      const total = typeof res.totalMinedCoins === 'number'
        ? res.totalMinedCoins
        : (typeof res.minedCoins === 'number' ? res.minedCoins : 0);
      const matured = typeof res.maturedMinedCoins === 'number'
        ? res.maturedMinedCoins
        : (typeof res.minedCoins === 'number' ? res.minedCoins : 0);
      const unmatured = typeof res.unmaturedMinedCoins === 'number'
        ? res.unmaturedMinedCoins
        : Math.max(0, total - matured);
  ...
  useEffect(() => {
    try {
      const compact = Array.isArray(log)
        ? log.slice(0, MAX_PERSISTED_LOG_ENTRIES)
        : [];
  ...
  const handleBlockMined = useCallback(async (minedInfo) => {
    if (!(window.wattcoinHardware && window.wattcoinHardware.invoke)) return;
    const payload = {
      blockHash: minedInfo && minedInfo.blockHash ? String(minedInfo.blockHash) : '',
      minedAddress: minedInfo && minedInfo.address
        ? String(minedInfo.address)
        : (selectedWalletAddress || ''),
      ...
      probeReceipt:        (minedInfo && minedInfo.probeReceipt && typeof minedInfo.probeReceipt === 'object')
                             ? minedInfo.probeReceipt : null,
      contributionsWh:     (minedInfo && minedInfo.contributionsWh && typeof minedInfo.contributionsWh === 'object')
                             ? minedInfo.contributionsWh : null,
    };
    try {
  ...
  useEffect(() => {
    ...
    function loadPurchaseTotal() {
      window.wattcoinHardware.invoke('wattcoin-sale-get-purchase-total', selectedWalletAddress)
        .then(res => { if (!cancelled && res && res.ok) setPurchasedWtc(Number(res.total) || 0); })
        .catch(() => {});
    }
    loadPurchaseTotal();
    ...
    <div style={{ minHeight: "100vh", background: "#060e06" }}>
      ...
      {updateReady && (
        <div style={{
          ...
          <span>{updateInstallRequested ? `Installing update v${updateReady.version}...` : `Update v${updateReady.version} ready to install`}</span>
          <button
            onClick={() => {
            ...
            style={{ background: "#4ade80", color: "#060e06", border: "none", borderRadius: 6, padding: "6px 18px", fontWeight: 700, fontSize: 14, cursor: updateInstallRequested ? "wait" : "pointer", opacity: updateInstallRequested ? 0.7 : 1 }}
          >
          ...
          <button
            ...
            disabled={updateInstallRequested}
            style={{ background: "transparent", color: "#86efac", border: "1px solid #4ade80", borderRadius: 6, padding: "5px 12px", fontWeight: 600, fontSize: 13, cursor: updateInstallRequested ? "default" : "pointer", opacity: updateInstallRequested ? 0.5 : 1 }}
          >
            Later
          </button>
        </div>
      ...
      <div style={{ paddingTop: `${TAB_BAR_HEIGHT_PX}px` }}>
        ...
        <div style={{ display: activeTab === "log" ? "block" : "none" }}>
          ...
          <MiningLog log={log} probeLog={probeLog} hwResetOnCooldown={hwResetOnCooldown} hwResetCooldownRemainingMs={hwResetCooldownRemainingMs} searchCacheOnCooldown={searchCacheOnCooldown} searchCacheCooldownRemainingMs={searchCacheCooldownRemainingMs} onClearLog={() => {
            setLog([]);
            setProbeLog([]);
            ...
            try {
              ...
              if (result && result.ok === false && result.reason === 'cooldown') {
                ...
                const timeStr = days > 0 ? `${days} day${days !== 1 ? 's' : ''} and ${hours} hour${hours !== 1 ? 's' : ''}` : `${hours} hour${hours !== 1 ? 's' : ''}`;
              ...
              const nextAllowedMs = result && result.nextClearAllowedAtMs ? result.nextClearAllowedAtMs : Date.now() + 3 * 24 * 60 * 60 * 1000;
              setSearchCacheOnCooldown(true);
            ...
            try {
              ...
              const nextAllowedMs = result && result.nextResetAllowedAtMs ? result.nextResetAllowedAtMs : Date.now() + 7 * 24 * 60 * 60 * 1000;
              setHwResetOnCooldown(true);
              setHwResetCooldownRemainingMs(nextAllowedMs - Date.now());
              const previousFingerprint = result && result.previousFingerprint && typeof result.previousFingerprint === 'object'
                ? result.previousFingerprint
                : null;
              const parts = [];
              if (previousFingerprint) {
                if (previousFingerprint.cpuModel) parts.push(`CPU ${previousFingerprint.cpuModel}`);
                if (Array.isArray(previousFingerprint.gpuModels) && previousFingerprint.gpuModels.length > 0) parts.push(`GPU ${previousFingerprint.gpuModels.join(', ')}`);
                if (previousFingerprint.memType || previousFingerprint.memSpeedMhz || previousFingerprint.memSticks) {
                  parts.push(`Memory ${previousFingerprint.memType || 'unknown'} ${previousFingerprint.memSpeedMhz || 0} MHz x${previousFingerprint.memSticks || 0}`);
                }
              }
              setLog((prev) => [{
                time: new Date().toLocaleString(),
...
function ProtectedTabUnlock({ busy, password, error, onPasswordChange, onSubmit }) {
  return (
    <div style={{ minHeight: `calc(100vh - ${TAB_BAR_HEIGHT_PX}px)`, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#060e06', padding: 24 }}>
      <form onSubmit={onSubmit} style={{ width: '100%', maxWidth: 420, background: '#060e06', border: '1px solid #1e3a1e', borderRadius: 14, padding: 24 }}>
        <div style={{ color: '#e8f5e8', fontSize: 18, fontWeight: 700, marginBottom: 8 }}>Password Required</div>
        <div style={{ color: '#7aaa7a', fontSize: 13, marginBottom: 16 }}>This machine must be unlocked once before Dashboard and Log are accessible.</div>
        <input
          type="password"
          value={password}
          ...
          style={{ width: '100%', boxSizing: 'border-box', background: '#0d160d', border: '1px solid #2d4a2d', borderRadius: 8, padding: '12px 14px', color: '#e8f5e8', fontFamily: 'monospace', fontSize: 14, outline: 'none', marginBottom: 12 }}
        />
        {error ? (
        ...
        <button
          type="submit"
          disabled={busy}
          style={{ width: '100%', background: busy ? '#1e3a1e' : '#4ade80', color: busy ? '#7aaa7a' : '#001008', border: 'none', borderRadius: 8, padding: '12px 16px', fontWeight: 700, fontSize: 14, cursor: busy ? 'default' : 'pointer' }}
        >
...
function WalletAddressDisplay({ selectedWalletAddress, walletSyncState, onAddressChange }) {
  ...
  const [addressNicknames, setAddressNicknames] = React.useState(() => {
    try { return JSON.parse(localStorage.getItem('wattcoin-address-nicknames') || '{}'); } catch (_) { return {}; }
  });
  ...
  function saveNicknames(updated) {
    ...
    try { localStorage.setItem('wattcoin-address-nicknames', JSON.stringify(updated)); } catch (_) {}
  }
  ...
  return (
    <div style={{ marginBottom: 18 }}>
            ...
            {editingNickname === address ? (
              <div style={{ display: 'flex', gap: 2, width: '100%' }}>
                <input
                  ...
                  onKeyDown={e => { if (e.key === 'Enter') commitNickname(); if (e.key === 'Escape') setEditingNickname(null); }}
                  placeholder="Nickname"
                  maxLength={20}
                  style={{ flex: 1, minWidth: 0, padding: '5px 6px', borderRadius: 6, border: '1px solid #4ade80', background: '#0d1a0d', color: '#d7ffd9', fontSize: 12, outline: 'none' }}
                />
                <button onClick={commitNickname} title="Save" style={{ background: 'none', border: 'none', color: '#4ade80', cursor: 'pointer', padding: '4px 4px', fontSize: 14, lineHeight: 1 }}>✓</button>
                <button onClick={() => setEditingNickname(null)} title="Cancel" style={{ background: 'none', border: 'none', color: '#9ac79f', cursor: 'pointer', padding: '4px 2px', fontSize: 14, lineHeight: 1 }}>✕</button>
              </div>
            ) : (
              <button
                ...
                addresses.map((addr, idx) => (
                  ...
                  >
                    {addressNicknames[addr] && (
                      <div style={{ fontSize: 11, fontWeight: 700, color: addr === address ? '#4ade80' : '#86efac', marginBottom: 2, fontFamily: 'inherit' }}>
                        {addressNicknames[addr]}
                      </div>
                    )}
                    <div style={{ fontFamily: "monospace", fontSize: 11, color: addr === address ? '#a7ffb0' : '#708870', wordBreak: 'break-all', lineHeight: 1.3 }}>
                      {addr}
                    </div>
                  </div>
                ))
      ...
      {createMessage && (
        <div style={{ marginTop: 8, fontSize: 12, color: createMessage.startsWith('Error') ? '#f87171' : '#86efac', padding: '4px 0' }}>
          {createMessage}
        </div>
      )}
      ...
      {showDeleteModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000 }}>
          <div style={{ background: '#241205', border: '2px solid #f59e0b', borderRadius: 12, padding: 24, minWidth: 360, maxWidth: 520, color: '#fef3c7' }}>
            <div style={{ fontSize: 17, fontWeight: 700, marginBottom: 10, color: '#f59e0b' }}>Delete Active Mining Address</div>
            <div style={{ fontSize: 13, marginBottom: 12, color: '#fde68a', whiteSpace: 'pre-wrap' }}>
              {`Active address:\n${address || '(none)'}`}
            </div>
            <div style={{ fontSize: 13, marginBottom: 12, color: '#fecaca' }}>
              Warning: this removes the currently active mining address from this wallet list. It is recoverable. Make sure you have a recovery backup file before continuing.
            </div>
            {deleteMessage && (
              <div style={{ fontSize: 12, marginBottom: 10, color: deleteMessage.startsWith('Address removed') ? '#4ade80' : '#fca5a5', whiteSpace: 'pre-wrap' }}>
                {deleteMessage}
              </div>
            )}
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button
                ...
                disabled={deleteBusy}
                style={{ padding: '7px 16px', borderRadius: 6, border: '1px solid #7c2d12', background: 'transparent', color: '#fcd34d', cursor: deleteBusy ? 'default' : 'pointer', fontWeight: 600, fontSize: 13 }}
              >
              ...
              {!deleteMessage.startsWith('Address removed') && (
                <button
                  ...
                  style={{ padding: '7px 16px', borderRadius: 6, border: 'none', background: (deleteBusy || !address) ? '#7c2d12' : '#f59e0b', color: (deleteBusy || !address) ? '#fcd34d' : '#1f1100', cursor: (deleteBusy || !address) ? 'default' : 'pointer', fontWeight: 700, fontSize: 13 }}
                >
      ...
      {showBackupModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000 }}>
          <div style={{ background: '#0a1a2a', border: '2px solid #60a5fa', borderRadius: 12, padding: 28, minWidth: 360, maxWidth: 460, color: '#e8f5e8' }}>
            ...
            <input
              ...
              onKeyDown={e => { if (e.key === 'Enter' && !backupBusy) doExportBackup(); }}
              style={{ width: '100%', padding: '8px 10px', borderRadius: 6, border: '1px solid #60a5fa', background: '#0d2233', color: '#e8f5e8', fontSize: 13, marginBottom: 12, boxSizing: 'border-box' }}
              autoFocus
              disabled={backupBusy}
            />
            {backupMessage && (
              <div style={{ fontSize: 12, marginBottom: 10, color: backupMessage.startsWith('Backup exported') ? '#4ade80' : '#f87171', whiteSpace: 'pre-wrap' }}>
                {backupMessage}
              </div>
            )}
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button
                ...
                disabled={backupBusy}
                style={{ padding: '7px 16px', borderRadius: 6, border: '1px solid #334155', background: 'transparent', color: '#94a3b8', cursor: backupBusy ? 'default' : 'pointer', fontWeight: 600, fontSize: 13 }}
              >
              ...
              {!backupMessage.startsWith('Backup exported') && (
                <button
                  ...
                  style={{ padding: '7px 16px', borderRadius: 6, border: 'none', background: (backupBusy || backupPassphrase.length < 8) ? '#1e3a5a' : '#60a5fa', color: (backupBusy || backupPassphrase.length < 8) ? '#4a6a8a' : '#001018', cursor: (backupBusy || backupPassphrase.length < 8) ? 'default' : 'pointer', fontWeight: 600, fontSize: 13 }}
                >
      ...
      {showRestoreModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000 }}>
          <div style={{ background: '#1a0a0a', border: '2px solid #fb7185', borderRadius: 12, padding: 28, minWidth: 360, maxWidth: 460, color: '#e8f5e8' }}>
            ...
            <div style={{ fontSize: 13, marginBottom: 14, color: '#fca5a5' }}>
              ⚠️ This will overwrite your current wallet and restart the node. Make sure you have a backup before proceeding.
            </div>
            <input
              type="password"
              ...
              onKeyDown={e => { if (e.key === 'Enter' && !restoreBusy) doRestoreBackup(restoreNeedsForce); }}
              style={{ width: '100%', padding: '8px 10px', borderRadius: 6, border: '1px solid #fb7185', background: '#2b0000', color: '#e8f5e8', fontSize: 13, marginBottom: 12, boxSizing: 'border-box' }}
              autoFocus
              disabled={restoreBusy}
            />
            {restoreMessage && (
              <div style={{ fontSize: 12, marginBottom: 10, color: restoreMessage.startsWith('Wallet restored') ? '#4ade80' : '#f87171', whiteSpace: 'pre-wrap' }}>
                {restoreMessage}
              </div>
            )}
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button
                ...
                disabled={restoreBusy}
                style={{ padding: '7px 16px', borderRadius: 6, border: '1px solid #334155', background: 'transparent', color: '#94a3b8', cursor: restoreBusy ? 'default' : 'pointer', fontWeight: 600, fontSize: 13 }}
              >
              ...
              {!restoreMessage.startsWith('Wallet restored') && (
                <button
                  ...
                  style={{ padding: '7px 16px', borderRadius: 6, border: 'none', background: (restoreBusy || restorePassphrase.length < 8) ? '#3a1010' : '#fb7185', color: (restoreBusy || restorePassphrase.length < 8) ? '#7a4040' : '#2b0000', cursor: (restoreBusy || restorePassphrase.length < 8) ? 'default' : 'pointer', fontWeight: 600, fontSize: 13 }}
                >
...
function ExplorerView({ blocks, total, offset, busy, status, selectedBlock, selectedBlockData, blockBusy, pageSize, onLoad, onSelectBlock }) {
    ...
    <div style={{ marginTop: 4 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          <button
            ...
            style={{ background: busy || !hasPrev ? '#1a2e1a' : '#1e3a1e', color: busy || !hasPrev ? '#456045' : '#b7f5bc', border: 'none', borderRadius: 6, padding: '5px 10px', fontWeight: 700, cursor: busy || !hasPrev ? 'default' : 'pointer', fontSize: 12 }}
          >
            ← Older
          </button>
          <button
            ...
            style={{ background: busy || !hasNext ? '#1a2e1a' : '#1e3a1e', color: busy || !hasNext ? '#456045' : '#b7f5bc', border: 'none', borderRadius: 6, padding: '5px 10px', fontWeight: 700, cursor: busy || !hasNext ? 'default' : 'pointer', fontSize: 12 }}
          >
            Newer →
          </button>
          <button
            onClick={() => onLoad(0)}
            disabled={busy}
            style={{ background: busy ? '#275a2f' : '#4ade80', color: busy ? '#9fd4a8' : '#001008', border: 'none', borderRadius: 6, padding: '5px 10px', fontWeight: 700, cursor: busy ? 'default' : 'pointer', fontSize: 12 }}
          >
            {busy ? 'Loading…' : 'Latest'}
          </button>
        </div>
      ...
      {total > 0 && (
        <div style={{ fontSize: 12, color: '#9ac79f', marginBottom: 8 }}>
          Chain height: <strong style={{ color: '#d7ffd9' }}>{total - 1}</strong> &nbsp;·&nbsp; {total} block{total !== 1 ? 's' : ''}
        </div>
      )}

      {status && (
        <div style={{ marginBottom: 8, fontSize: 13, color: '#fca5a5' }}>{status}</div>
      )}
          ...
          <div key={b.height}>
            ...
            >
              ...
              <div style={{ minWidth: 0 }}>
                <div style={{ fontSize: 11, color: '#9ac79f', fontFamily: 'monospace', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {shortHash(b.hash)}
                </div>
                <div style={{ fontSize: 11, color: '#708870', marginTop: 2 }}>
                  {formatTs(b.timestamp)} &nbsp;·&nbsp; {b.txCount} tx &nbsp;·&nbsp; {b.proposer === 'genesis' ? 'genesis' : (b.proposer ? b.proposer.slice(0, 12) + '…' : '-')}
                </div>
              </div>
            ...
            {selectedBlock === b.height && (
              <div style={{ background: '#0a150a', border: '1px solid #4ade80', borderTop: 'none', borderRadius: '0 0 8px 8px', padding: '10px 14px' }}>
                {blockBusy ? (
...
function BlockDetail({ block }) {
  ...
  return (
    <div style={{ fontSize: 12, display: 'grid', gap: 5 }}>
      ...
      <Row label="Energy"   value={`${(block.energyWh || 0).toLocaleString(undefined, { maximumFractionDigits: 4 })} Wh`} />
      <Row label="Reward"   value={`${(block.rewardTotal || 0).toLocaleString(undefined, { maximumFractionDigits: 8 })} WTC`} />
      {rewardEntries.length > 0 && (
        <div style={{ marginTop: 4 }}>
          <div style={{ color: '#4ade80', fontWeight: 700, marginBottom: 3 }}>Reward distribution</div>
          {rewardEntries.map(([addr, amt]) => (
            <div key={addr} style={{ display: 'flex', justifyContent: 'space-between', color: '#9ac79f', fontFamily: 'monospace', fontSize: 11, wordBreak: 'break-all' }}>
              <span style={{ marginRight: 8, flex: 1 }}>{addr}</span>
              <span style={{ whiteSpace: 'nowrap', color: '#86efac' }}>{amt.toLocaleString(undefined, { maximumFractionDigits: 8 })} WTC</span>
            </div>
          ))}
        </div>
      )}
      {txs.length > 0 && (
        <div style={{ marginTop: 4 }}>
          ...
          {txs.map((tx, i) => (
            <div key={tx.id || i} style={{ background: '#0d1a0d', border: '1px solid #1e3a1e', borderRadius: 6, padding: '6px 10px', marginBottom: 4 }}>
              <div style={{ color: '#9ac79f', fontFamily: 'monospace', fontSize: 11, wordBreak: 'break-all' }}>ID: {tx.id || '-'}</div>
      ...
      {votes.length > 0 && (
        <div style={{ marginTop: 4 }}>
          ...
          {votes.map((v) => (
            <div key={v} style={{ fontSize: 11, color: '#708870', fontFamily: 'monospace', wordBreak: 'break-all' }}>{v}</div>
          ))}
        </div>
      )}
    </div>
...
function Row({ label, value, mono }) {
  return (
    <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
      ...
      <span style={{ color: '#d7ffd9', fontFamily: mono ? 'monospace' : undefined, fontSize: mono ? 11 : 12, wordBreak: 'break-all' }}>{String(value ?? '-')}</span>
    </div>
  );
}
...
function WalletPayModal({ orderId, usdcRequired, wtcAmount, onPaid, onManual, onClose }) {
  ...
  async function handleInjected() {
    const eth = window.ethereum;
    if (!eth) { setErrorMsg('No injected wallet detected.'); return; }
    ...
    try {
      ...
      if (e && e.code === 4001) { setStep('pick'); setErrorMsg('Transaction cancelled.'); }
      else { setStep('error'); setErrorMsg(e && e.message ? e.message : 'Unknown error'); }
    }
  }
  ...
  const btn = { display: 'flex', alignItems: 'center', gap: 14, width: '100%', padding: '14px 16px', borderRadius: 12, cursor: 'pointer', color: '#e8f5e8', textAlign: 'left', background: '#0a1a0a', border: '1px solid #2d4a2d' };

  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,6,0,0.84)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ background: '#0d1a0d', border: '1px solid #2d4a2d', borderRadius: 18, padding: '28px 28px', maxWidth: 400, width: '100%', position: 'relative' }}>
        <button onClick={onClose} style={{ position: 'absolute', top: 12, right: 16, background: 'none', border: 'none', color: '#4a6a4a', fontSize: 22, cursor: 'pointer', lineHeight: 1 }}>×</button>
        ...
        {step === 'pick' && (
          <>
            ...
            <div style={{ fontSize: 13, color: '#6b9b6b', marginBottom: 20 }}>
              Send <span style={{ color: '#fcd34d', fontWeight: 700 }}>${usdcRequired.toFixed(2)} USDC</span> to receive <span style={{ color: '#a7ffb0', fontWeight: 700 }}>{wtcAmount?.toLocaleString()} WTC</span>
            </div>
        ...
        {(step === 'connecting' || step === 'confirm') && (
          <div style={{ textAlign: 'center', padding: '28px 0' }}>
            ...
            <div style={{ fontSize: 12, color: '#6b9b6b' }}>
              {step === 'connecting' ? 'Approve the connection in your wallet' : `Review and confirm the $${usdcRequired.toFixed(2)} USDC transfer`}
            </div>
          </div>
        )}
        ...
        {step === 'done' && (
          <div style={{ textAlign: 'center', padding: '24px 0' }}>
            ...
            <div style={{ fontSize: 12, color: '#6b9b6b', marginBottom: 10 }}>Your WTC will appear in your wallet once the batch of 10,101 WTC is queued.</div>
            <div style={{ fontFamily: 'monospace', fontSize: 10, color: '#3d5c3d', wordBreak: 'break-all' }}>{txHash}</div>
          </div>
        )}
        ...
        {step === 'error' && (
          <>
            ...
            <button onClick={() => { setStep('pick'); setErrorMsg(''); }} style={{ background: '#1e3a1e', color: '#b7f5bc', border: 'none', borderRadius: 8, padding: '9px 18px', cursor: 'pointer', fontSize: 12, fontWeight: 700 }}>
              Try Again
            </button>
          </>
        )}
...
function StakingView({ selectedWalletAddress, walletBalance, queuedWtc = 0 }) {
  ...
  async function handleStake() {
    ...
    if (!address) { setMsg('No wallet address selected.'); setMsgOk(false); return; }
    ...
    try {
      ...
      if (r.ok) {
        setMsg(r.alreadyExists ? 'You already have a pending staking entry.' : 'Stake submitted! Reward will be paid when queue reaches the flush threshold.');
        setMsgOk(true);
        setStakeAmount('');
        await refresh();
      } else {
  ...
  return (
    <div style={{ padding: '18px 0' }}>
      {/* ── Wallet balance ── */}
      <div style={{
        background: '#071507', border: '1px solid #1a3a1a', borderRadius: 10,
        padding: '11px 18px', marginBottom: 16,
        display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap'
      }}>
        <div style={{ color: '#6ee688', fontSize: 11, fontWeight: 600, letterSpacing: '0.06em' }}>YOUR WALLET BALANCE</div>
        <div style={{ color: (walletBalance + queuedWtc) > 0 ? '#4ade80' : '#9ca3af', fontSize: 16, fontWeight: 800, marginLeft: 'auto' }}>
      ...
      {/* ── Pool info bar ── */}
      <div style={{
        background: '#0d2710', borderRadius: 10, padding: '14px 18px',
        marginBottom: 18, display: 'flex', gap: 32, flexWrap: 'wrap'
      }}>
        <div>
        ...
        <div>
          ...
          <div style={{ color: '#e2f5e5', fontSize: 20, fontWeight: 700 }}>
            {totalStaked.toLocaleString()} WTC
          </div>
        </div>
        <div>
          <div style={{ color: '#6ee688', fontSize: 11, fontWeight: 600, marginBottom: 2 }}>CURRENT APY</div>
          <div style={{ color: apy > 0 ? '#4ade80' : '#9ca3af', fontSize: 20, fontWeight: 700 }}>
            {apy.toFixed(2)}%
          </div>
        </div>
      </div>
      ...
      <div style={{ color: '#9ca3af', fontSize: 12, marginBottom: 16, lineHeight: 1.5 }}>
        Stake WTC to earn rewards. Every <strong style={{ color: '#b7f5bc' }}>100&nbsp;WTC</strong> in the queue adds <strong style={{ color: '#b7f5bc' }}>0.01% APY</strong>.
        Staking will end when the total supply of <strong style={{ color: '#b7f5bc' }}>166,667&nbsp;WTC</strong> has been distributed.
      </div>
      ...
      {!hasPending ? (
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 14, flexWrap: 'wrap' }}>
          <input
            ...
            disabled={busy}
            style={{
              background: '#0d2710', border: '1.5px solid #265b2e', borderRadius: 8,
              color: '#e2f5e5', padding: '9px 13px', fontSize: 14, width: 220,
              outline: 'none'
            }}
          />
          <button
        ...
        <div style={{
          background: '#0d2710', borderRadius: 8, padding: '10px 14px',
          marginBottom: 14, color: '#4ade80', fontSize: 13
        }}>
      ...
      {msg && (
        <div style={{
          ...
          color: msgOk ? '#4ade80' : '#f87171', fontSize: 13
        }}>
          {msg}
        </div>
      )}
      ...
      {myEntries.length > 0 && (
        <div>
          <div style={{ color: '#6ee688', fontSize: 13, fontWeight: 700, marginBottom: 8 }}>
            My Staking Entries
          </div>
          {myEntries.map(entry => (
            <div key={entry.id} style={{
              background: '#0d2710', borderRadius: 8, padding: '12px 14px',
              ...
              <div style={{ minWidth: 80 }}>
                <span style={{
                  background: entry.status === 'pending' ? '#1a3a2a' :
                              entry.status === 'rewarded' ? '#0d2710' : '#2a1a1a',
                  color: entry.status === 'pending' ? '#4ade80' :
                         entry.status === 'rewarded' ? '#86efac' : '#f87171',
...
function SaleView({ selectedWalletAddress }) {
  ...
  React.useEffect(() => {
    if (!(window.wattcoinHardware && window.wattcoinHardware.invoke)) return;
    window.wattcoinHardware.invoke('wattcoin-get-addresses')
      .then(res => {
  ...
  const activeTierIdx = sold === null ? 0
    : sold >= SALE_TOTAL         ? -1
    : sold < SALE_TIER_SIZE      ? 0
    : sold < 2 * SALE_TIER_SIZE  ? 1
    : 2;
  ...
  React.useEffect(() => {
    ...
    window.wattcoinHardware.invoke('wattcoin-sale-get-my-orders', buyToAddress)
      .then(res => {
  ...
  React.useEffect(() => {
    ...
    window.wattcoinHardware.invoke('wattcoin-sale-compute-price', {
      wtcAmount: amount,
        ...
        if (r.alreadyExists) {
          ...
          try {
            ...
            if (ord && ord.ok && ord.order) {
              ...
              setOrderStatus({ status: resolvedStatus, wtcAmount: r.wtcAmount || amount, usdcRequired: r.usdcRequired, id: r.orderId });
            }
          } catch (_) {
            setOrderStatus({ status: resolvedStatus, wtcAmount: r.wtcAmount || amount, usdcRequired: r.usdcRequired, id: r.orderId });
          }
  ...
  function handleCancelOrder() {
    ...
    window.wattcoinHardware.invoke('wattcoin-sale-cancel-order', cancellingId)
      .then(r => {
        if (r && r.ok) {
          loadSaleData();
          if (orderPollRef.iv) { clearInterval(orderPollRef.iv); orderPollRef.iv = null; }
          setOrderId(null);
          setOrderStatus(null);
  ...
  function copyUsdc() {
    navigator.clipboard.writeText(SELLER_USDC).then(() => {
      setCopied(true);
  ...
  return (
    <div style={{ marginTop: 4 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
        ...
        {lastEtherscanPoll && lastEtherscanPoll.at && (() => {
          ...
          return (
            <div title={`Last Etherscan USDC check: ${label}`} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 10, color: '#4a6a4a', fontFamily: 'monospace' }}>
        ...
        {electricityPrice !== null && (
          <div
            ...
            style={{ display: 'flex', alignItems: 'center', gap: 5, background: '#0a1f0a', border: '1px solid #1e3a1e', borderRadius: 6, padding: '4px 10px', cursor: 'default' }}
          >
      ...
      {sold !== null && (
        <div style={{ marginBottom: 22 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#6b9b6b', marginBottom: 5 }}>
            <span>{sold.toLocaleString()} WTC sold</span>
            <span>{Math.max(0, SALE_TOTAL - sold).toLocaleString()} WTC remaining</span>
          </div>
          <div style={{ height: 10, background: '#122612', borderRadius: 999, overflow: 'hidden', border: '1px solid #1e3a1e', position: 'relative' }}>
            {[1/3, 2/3].map((frac) => (
              <div key={frac} style={{ position: 'absolute', left: `${frac * 100}%`, top: 0, bottom: 0, width: 1, background: '#4ade8040', zIndex: 1 }} />
            ))}
            <div style={{ height: '100%', width: `${Math.min(100, (sold / SALE_TOTAL) * 100)}%`, background: 'linear-gradient(90deg, #4ade80, #22c55e)', transition: 'width 0.4s ease', borderRadius: 999 }} />
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: '#4a6a4a', marginTop: 3 }}>
            <span>0</span><span>111,111</span><span>222,222</span><span>333,333</span>
          </div>
        </div>
      )}
        ...
        {SALE_TIERS.map((t) => {
          ...
          return (
            <div key={t.idx} style={{
              ...
              opacity: isCompleted ? 0.65 : 1, transition: 'border-color 0.2s',
            }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ fontWeight: 700, fontSize: 14, color: isActive ? '#4ade80' : '#9ac79f' }}>{t.label}</span>
                  {isActive    && <span style={{ fontSize: 10, background: '#4ade80', color: '#001008', borderRadius: 999, padding: '2px 7px', fontWeight: 700 }}>ACTIVE</span>}
                  {isCompleted && <span style={{ fontSize: 10, background: '#2d4a2d', color: '#4a9a4a', borderRadius: 999, padding: '2px 7px', fontWeight: 700 }}>SOLD OUT</span>}
                </div>
                <div style={{ textAlign: 'right' }}>
                  <span style={{ fontFamily: 'monospace', fontSize: 13, color: '#fcd34d', fontWeight: 700 }}>
                    {usdPrice !== null ? (usdPrice >= 1 ? `~$${usdPrice.toFixed(2)}` : `~$${usdPrice.toFixed(4)}`) : `${Math.round(t.fraction * 100)}%`}
                  </span>
              ...
              <div style={{ height: 6, background: '#122612', borderRadius: 999, overflow: 'hidden', border: '1px solid #1e3a1e' }}>
                <div style={{ height: '100%', width: `${tierPct}%`, background: isCompleted ? '#2d4a2d' : 'linear-gradient(90deg, #4ade80, #22c55e)', transition: 'width 0.4s ease', borderRadius: 999 }} />
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: '#4a6a4a', marginTop: 3 }}>
      ...
      {!orderStatus ? (
        <div style={{ background: '#0a1f0a', border: '1px solid #1e3a1e', borderRadius: 14, padding: '18px 20px' }}>
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={{ display: 'block', marginBottom: 6, fontSize: 11, color: '#9ac79f', letterSpacing: '0.06em', fontFamily: 'monospace' }}>DELIVER WTC TO</label>
            {localAddresses.length > 1 ? (
              <select
                value={buyToAddress}
                onChange={e => setBuyToAddress(e.target.value)}
                style={{ width: 'auto', background: '#0d160d', border: '1px solid #2d4a2d', borderRadius: 8, padding: '9px 12px', color: '#a7ffb0', fontFamily: 'monospace', fontSize: 11, outline: 'none', cursor: 'pointer' }}
              >
                {localAddresses.map(addr => <option key={addr} value={addr}>{addr}</option>)}
              </select>
            ) : (
              <div style={{ display: 'inline-block', fontFamily: 'monospace', fontSize: 11, color: buyToAddress ? '#a7ffb0' : '#fca5a5', background: '#0d160d', border: `1px solid ${buyToAddress ? '#1e3a1e' : '#4a1a1a'}`, borderRadius: 8, padding: '9px 12px' }}>
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={{ display: 'block', marginBottom: 6, fontSize: 11, color: '#9ac79f', letterSpacing: '0.06em', fontFamily: 'monospace' }}>YOUR USDC WALLET ADDRESS <span style={{ color: '#fbbf24' }}>(required)</span></label>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <input
                ...
                style={{
                  width: '44ch',
                  background: '#0d160d', border: `1px solid ${buyerEthAddress && (!buyerEthAddress.startsWith('0x') || buyerEthAddress.length < 40) ? '#4a1a1a' : '#2d4a2d'}`,
                  borderRadius: 8, padding: '9px 12px',
                  color: '#a7ffb0', fontFamily: 'monospace', fontSize: 11, outline: 'none',
                }}
              />
              <button
                onClick={() => navigator.clipboard.readText().then(t => setBuyerEthAddress(t.trim())).catch(() => {})}
                style={{ background: '#1e3a1e', color: '#a7ffb0', border: '1px solid #2d4a2d', borderRadius: 8, padding: '9px 12px', fontSize: 11, cursor: 'pointer', whiteSpace: 'nowrap' }}
              >Paste</button>
            </div>
            <div style={{ fontSize: 11, color: '#4a6a4a', marginTop: 4 }}>The Ethereum address you will send USDC from — used to match your payment automatically.</div>
          </div>
          ...
          <input
            ...
            style={{
              ...
              background: '#0d160d', border: '1px solid #2d4a2d',
              borderRadius: 8, padding: '10px 12px',
              color: '#a7ffb0', fontFamily: 'monospace', fontSize: 14,
              outline: 'none',
            }}
          />
          ...
          {usdcRequired && (
            <div style={{ fontSize: 11, color: '#6b9b6b', marginBottom: 10, lineHeight: 1.5, background: '#0d160d', border: '1px solid #1e3a1e', borderRadius: 8, padding: '8px 12px' }}>
              WTC will be sent to your wallet once 10,101 WTC is queued.
            </div>
          )}

          <button
            disabled={!usdcRequired || !buyToAddress || priceLoading || !buyerEthAddress || !buyerEthAddress.startsWith('0x') || buyerEthAddress.length < 40}
            onClick={handlePlaceOrder}
            style={{
              width: '100%', padding: '12px 0', borderRadius: 10, border: 'none',
              background: (!usdcRequired || !buyToAddress || priceLoading || !buyerEthAddress || !buyerEthAddress.startsWith('0x') || buyerEthAddress.length < 40) ? '#1e3a1e' : '#4ade80',
              color:       (!usdcRequired || !buyToAddress || priceLoading || !buyerEthAddress || !buyerEthAddress.startsWith('0x') || buyerEthAddress.length < 40) ? '#4a6a4a'  : '#001008',
              fontWeight: 700, fontSize: 14, cursor: (!usdcRequired || !buyToAddress || priceLoading || !buyerEthAddress || !buyerEthAddress.startsWith('0x') || buyerEthAddress.length < 40) ? 'not-allowed' : 'pointer',
              transition: 'background 0.2s',
            }}
          >
            {priceLoading ? 'Computing...' : !buyerEthAddress ? 'Enter your USDC wallet address' : usdcRequired ? `Place Order — Pay $${usdcRequired.toFixed(2)} USDC` : 'Enter amount to see price'}
          </button>
        </div>
      ) : (
        /* ── Active order card ─────────────────────────────────────────── */
        <div style={{ background: '#0a1f0a', border: `1px solid ${orderStatus.status === 'fulfilled' ? '#4ade80' : (optimisticPaid || orderStatus.status === 'payment_submitted') ? '#a7ffb0' : '#fbbf24'}`, borderRadius: 14, padding: '18px 20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: orderStatus.status === 'fulfilled' ? '#4ade80' : (optimisticPaid || orderStatus.status === 'payment_submitted') ? '#a7ffb0' : '#fbbf24' }}>
              {(optimisticPaid || orderStatus.status === 'payment_submitted') && orderStatus.status !== 'queued' && orderStatus.status !== 'fulfilled' && 'Payment Submitted — Pending Delivery'}
          ...
          <div style={{ fontSize: 13, color: '#9ac79f', marginBottom: 14 }}>
            {orderStatus.wtcAmount?.toLocaleString()} WTC &rarr; <span style={{ fontFamily: 'monospace' }}>{orderStatus.wtcAddress || selectedWalletAddress}</span>
          </div>

          {orderStatus.status === 'pending_payment' && !optimisticPaid && (
            <>
              <div style={{ padding: '12px 14px', background: '#091409', border: '1px solid #2d4a2d', borderRadius: 10, marginBottom: 14 }}>
                ...
                <div style={{ fontSize: 11, color: '#6b9b6b', marginBottom: 8 }}>
                  Send exactly <span style={{ color: '#fcd34d', fontWeight: 700 }}>${orderStatus.usdcRequired?.toFixed(2)} USDC</span> (Ethereum mainnet ERC-20) to:
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#0d160d', border: '1px solid #2d4a2d', borderRadius: 6, padding: '7px 10px', marginBottom: 8 }}>
                  <span style={{ fontFamily: 'monospace', fontSize: 11, color: '#a7ffb0', wordBreak: 'break-all', flex: 1 }}>{SELLER_USDC}</span>
                  <button
                    onClick={copyUsdc}
                    style={{ background: copied ? '#4ade80' : '#1e3a1e', color: copied ? '#001008' : '#b7f5bc', border: 'none', borderRadius: 6, padding: '3px 8px', fontSize: 11, fontWeight: 700, cursor: 'pointer', flexShrink: 0 }}
                  >
                    {copied ? 'Copied!' : 'Copy'}
                  </button>
                </div>
              ...
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                <button
                  ...
                  style={{ background: '#1e3a1e', color: '#a7ffb0', border: '1px solid #4ade80', borderRadius: 8, padding: '9px 18px', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}
                >
                  Pay with Wallet
                </button>
                <button
                  onClick={handleCancelOrder}
                  style={{ background: 'transparent', color: '#6b9b6b', border: '1px solid #2d4a2d', borderRadius: 8, padding: '9px 14px', fontSize: 13, cursor: 'pointer' }}
                >
                  Cancel Order
                </button>
              </div>
          ...
          {(orderStatus.status === 'queued' || orderStatus.status === 'payment_submitted' || (optimisticPaid && orderStatus.status === 'pending_payment')) && (
            <div style={{ padding: '12px 14px', background: '#091409', border: '1px solid #2d4a2d', borderRadius: 10 }}>
              ...
              <div style={{ fontSize: 11, color: '#6b9b6b' }}>Payment confirmed — WTC will be sent once 10,101 WTC is queued network-wide.</div>
              {(orderStatus.status === 'payment_submitted' || (optimisticPaid && orderStatus.status === 'pending_payment')) && (
                <div style={{ fontSize: 11, color: '#7a6a3a', marginTop: 4 }}>Payment detection may take up to 10 minutes.</div>
              )}
            </div>
          )}
          ...
          {(orderStatus.status === 'queued' || orderStatus.status === 'payment_submitted' || (optimisticPaid && orderStatus.status === 'pending_payment') || orderStatus.status === 'fulfilled' || orderStatus.status === 'failed' || orderStatus.status === 'expired' || orderStatus.status === 'cancelled') && (
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 14 }}>
              <button
                onClick={() => { if (orderPollRef.iv) { clearInterval(orderPollRef.iv); orderPollRef.iv = null; } setOrderId(null); setOrderStatus(null); setOrderMsg(''); setOrderError(''); setBuyAmount(''); setUsdcRequired(null); setOptimisticPaid(false); setShowWalletModal(false); }}
                style={{ background: '#1e3a1e', color: '#b7f5bc', border: 'none', borderRadius: 8, padding: '9px 18px', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}
              >
                {(orderStatus.status === 'queued' || orderStatus.status === 'payment_submitted' || (optimisticPaid && orderStatus.status === 'pending_payment')) ? 'Place New Order' : 'Place Another Order'}
              </button>
              {(orderStatus.status === 'queued' || orderStatus.status === 'payment_submitted' || (optimisticPaid && orderStatus.status === 'pending_payment')) && (
                <button
                  onClick={handleCancelOrder}
      ...
      {showWalletModal && orderStatus && (
        <WalletPayModal
          ...
          onPaid={() => { setOptimisticPaid(true); setShowWalletModal(false); }}
          onManual={() => { setOptimisticPaid(true); setShowWalletModal(false); }}
...
function NftTierStatusBar({ purchasedWtc = 0 }) {
  ...
  if (!hasBronze)      { toGo = NFT_TIER_BRONZE - purchasedWtc; toGoTier = 'Bronze'; toGoColor = '#d97706'; }
  else if (!hasSilver) { toGo = NFT_TIER_SILVER - purchasedWtc; toGoTier = 'Silver'; toGoColor = '#9ca3af'; }
  else if (!hasGold)   { toGo = NFT_TIER_GOLD   - purchasedWtc; toGoTier = 'Gold';   toGoColor = '#fbbf24'; }
  ...
  return (
    <div style={{ background: '#0a140a', border: '1px solid #1e3a1e', borderRadius: 12, padding: '11px 16px', marginBottom: 14 }}>
      {/* Header row: label + amount + badges | thresholds */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8, gap: 8, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, color: '#4ade80', letterSpacing: '0.1em', whiteSpace: 'nowrap' }}>PURCHASED WTC</span>
          <span style={{ fontSize: 15, fontWeight: 700, color: '#e8f5e8', fontFamily: 'monospace', whiteSpace: 'nowrap' }}>
        ...
        <div style={{ display: 'flex', gap: 8, fontSize: 9, fontFamily: "'DM Mono', monospace", flexShrink: 0, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
      ...
      <div style={{ height: 7, background: '#172a17', borderRadius: 99, overflow: 'hidden', position: 'relative' }}>
        {/* Tier marker lines */}
        <div style={{ position: 'absolute', left: '20%', top: 0, bottom: 0, width: 1, background: '#d97706', opacity: 0.45, zIndex: 1, pointerEvents: 'none' }} />
        <div style={{ position: 'absolute', left: '60%', top: 0, bottom: 0, width: 1, background: '#9ca3af', opacity: 0.45, zIndex: 1, pointerEvents: 'none' }} />
        {/* Fill */}
        <div style={{
      ...
      <div style={{ marginTop: 5, fontSize: 11, minHeight: 16 }}>
        {toGo !== null ? (
          <>
            <span style={{ fontFamily: 'monospace', fontWeight: 700, color: toGoColor }}>{Math.ceil(toGo).toLocaleString()}</span>
...
function WalletTab({ coins, maturedCoins, unmaturedCoins, energy, selectedWalletAddress, walletSyncState, refreshBalances, onAddressChange, purchasedWtc = 0 }) {
  ...
  const walletReadiness = walletSyncState && walletSyncState.walletReadiness
  ...
  React.useEffect(() => {
    try {
      const compact = Array.isArray(sentTxHistory)
        ? sentTxHistory.slice(0, MAX_PERSISTED_SENT_TXS)
        : [];
  ...
  const loadNfts = React.useCallback(() => {
    ...
    setNftsBusy(true);
    setNftTransferMsg('');
    if (!selectedWalletAddress) { setNftsBusy(false); return; }
    window.wattcoinHardware.invoke('wattcoin-nft-list', selectedWalletAddress)
      .then(res => { if (res && res.ok) setNfts(res.nfts || []); })
      .catch(() => {})
  ...
  React.useEffect(() => {
    ...
    async function loadBetaPolicy() {
      if (!(window.wattcoinHardware && window.wattcoinHardware.invoke)) {
        if (!cancelled) {
          setBetaPolicy({ loading: false, betaMode: false, withdrawalsEnabled: true, resetWipePolicy: false, policyMessage: '' });
        }
        return;
      }
      try {
  ...
  React.useEffect(() => {
    ...
    let cancelled = false;
    function loadQueued() {
      window.wattcoinHardware.invoke('wattcoin-sale-get-my-orders', selectedWalletAddress)
        .then(res => {
  ...
  function handleWithdrawAddressChange(raw) {
    ...
    if (!trimmed) { setAddrValidation({ state: 'empty', reason: '' }); return; }
  ...
  function _stopTxPoll() {
    if (txStatusPollRef.current)    { clearInterval(txStatusPollRef.current);   txStatusPollRef.current    = null; }
    if (txStatusTimeoutRef.current) { clearTimeout(txStatusTimeoutRef.current);  txStatusTimeoutRef.current = null; }
  }
    ...
    try {
      ...
      if (res && res.ok) {
        setSentTxHistory((prev) => [{
          ...
          selectedAddress: selectedWalletAddress || '',
          localOnly: true,
        }, ...prev.filter((tx) => tx && tx.txid !== res.txid)].slice(0, MAX_PERSISTED_SENT_TXS));
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 14, flexWrap: 'nowrap' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 4, minWidth: 0 }}>
          ...
          <span style={{ fontSize: 11, color: networkInfo.network === 'testnet' ? '#fbbf24' : '#9ac79f', background: '#1e3a1e', borderRadius: 999, padding: '4px 8px', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase' }}>
            {networkInfo.network}
          </span>
        </div>
        ...
        <div style={{
          ...
          {betaPolicy.resetWipePolicy && (
            <div style={{ fontSize: 11, color: '#fcd34d' }}>
              Beta balances may be reset or wiped at any time.
            </div>
          )}
        </div>
      )}

      {queuedSaleOrders.filter(o => o.status === 'pending_payment' || o.status === 'payment_submitted').length > 0 && (
        <div style={{ marginTop: 8, marginBottom: 4, padding: '10px 14px', background: '#12100a', border: '1px solid #3d3000', borderRadius: 10, display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          <span style={{ fontFamily: 'monospace', fontSize: 10, color: '#fbbf24', letterSpacing: '0.1em' }}>SALE QUEUE</span>
          <span style={{ fontFamily: 'monospace', fontSize: 15, color: '#fcd34d', fontWeight: 700 }}>
            +{queuedSaleOrders.filter(o => o.status === 'pending_payment' || o.status === 'payment_submitted').reduce((s, o) => s + (o.wtcAmount || 0), 0).toLocaleString()} WTC
          </span>
          <span style={{ fontSize: 11, color: '#7a6a3a' }}>
            {queuedSaleOrders.some(o => o.status === 'payment_submitted') ? 'paid — awaiting confirmation (up to 10 min)' : 'awaiting payment — up to 10 min to detect'}
          </span>
        </div>
      )}
      ...
      {walletView === 'overview' && (
        <>
          <div style={{ marginBottom: 8 }}>
            ...
            <span style={{ fontFamily: "monospace", marginLeft: 12, fontSize: 20, background: "#1e3a1e", padding: "4px 10px", borderRadius: 8 }}>
          ...
          {(() => {
            ...
            return (
              <div style={{ marginBottom: 8 }}>
                ...
                <span style={{ fontFamily: 'monospace', marginLeft: 12, fontSize: 20, background: '#12100a', border: '1px solid #3d3000', padding: '4px 10px', borderRadius: 8, color: '#fcd34d' }}>
          ...
          <div style={{ marginTop: 24, paddingTop: 18, borderTop: '1px solid #1e3a1e' }}>
            ...
            <div style={{ marginBottom: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                ...
                {addrValidation.state === 'invalid' && (
                  <span style={{ fontSize: 11, color: '#f87171', fontWeight: 700 }}>✕ {addrValidation.reason || 'Invalid address'}</span>
                )}
              </div>
              <input
                type="text"
                ...
                style={{
                  ...
                  background: '#0d1a0d', color: '#d7ffd9',
                  border: `1px solid ${addrValidation.state === 'valid' ? '#4ade80' : addrValidation.state === 'invalid' ? '#f87171' : '#2a4a2a'}`,
                  borderRadius: 8, padding: '9px 10px',
                }}
                disabled={withdrawBusy || betaPolicy.betaMode || !betaPolicy.withdrawalsEnabled || !walletReadiness.spendReady}
              />
            </div>
            ...
            <div style={{ marginBottom: 10 }}>
              ...
              <input
                ...
                style={{ width: '100%', boxSizing: 'border-box', fontFamily: 'monospace', fontSize: 13, background: '#0d1a0d', color: '#d7ffd9', border: '1px solid #2a4a2a', borderRadius: 8, padding: '9px 10px' }}
                disabled={withdrawBusy || betaPolicy.betaMode || !betaPolicy.withdrawalsEnabled || !walletReadiness.spendReady}
              />
            </div>

            <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: '#9ac79f', marginBottom: 12, cursor: withdrawBusy ? 'default' : 'pointer' }}>
              <input
                type="checkbox"
            ...
            <button
              ...
              disabled={withdrawBusy || betaPolicy.betaMode || !betaPolicy.withdrawalsEnabled || !walletReadiness.spendReady || addrValidation.state !== 'valid'}
              style={{
                background: (withdrawBusy || betaPolicy.betaMode || !betaPolicy.withdrawalsEnabled || !walletReadiness.spendReady || addrValidation.state !== 'valid') ? '#275a2f' : '#4ade80',
                color: (withdrawBusy || betaPolicy.betaMode || !betaPolicy.withdrawalsEnabled || !walletReadiness.spendReady || addrValidation.state !== 'valid') ? '#9fd4a8' : '#001008',
                border: 'none', borderRadius: 8, padding: '9px 14px', fontWeight: 700,
                cursor: (withdrawBusy || betaPolicy.betaMode || !betaPolicy.withdrawalsEnabled || !walletReadiness.spendReady || addrValidation.state !== 'valid') ? 'default' : 'pointer',
                fontSize: 13
              }}
            >
              {betaPolicy.betaMode || !betaPolicy.withdrawalsEnabled ? 'Withdrawals Disabled In Beta' : (!walletReadiness.spendReady ? 'Withdrawals Locked Until Sync Ready' : (withdrawBusy ? 'Sending...' : 'Send Withdrawal'))}
            </button>

            {withdrawMessage && (
              <div style={{ marginTop: 10, fontSize: 13, display: 'flex', alignItems: 'center', gap: 7,
                color: withdrawMessage.startsWith('Transaction confirmed') ? '#4ade80'
                     : withdrawMessage.startsWith('Transaction failed')   ? '#fca5a5'
                     : '#fcd34d' }}>
                {withdrawPending && (
                  <span style={{ display: 'inline-block', width: 11, height: 11, borderRadius: '50%',
                    border: '2px solid #fcd34d', borderTopColor: 'transparent',
          ...
          {showSendConfirm && (
            <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.82)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 3000 }}>
              <div style={{ background: '#0f1f0f', border: '2px solid #f59e0b', borderRadius: 14, padding: 28, minWidth: 360, maxWidth: 520, color: '#fef3c7' }}>
                <div style={{ fontSize: 17, fontWeight: 700, color: '#f59e0b', marginBottom: 14 }}>⚠ Confirm Withdrawal</div>
                ...
                <div style={{ fontSize: 12, fontFamily: 'monospace', color: '#d7ffd9', background: '#0d1a0d', border: '1px solid #2a4a2a', borderRadius: 6, padding: '8px 10px', wordBreak: 'break-all', marginBottom: 14 }}>
                  {(withdrawAddress || '').trim()}
                </div>
                {subtractFeeFromAmount && (
                  <div style={{ fontSize: 12, color: '#fcd34d', marginBottom: 10 }}>Network fee will be subtracted from the sent amount.</div>
                )}
                ...
                <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                  <button
                    ...
                    style={{ padding: '8px 18px', borderRadius: 7, border: '1px solid #7c2d12', background: 'transparent', color: '#fcd34d', cursor: 'pointer', fontWeight: 600, fontSize: 13 }}
                  >
                    Cancel
                  </button>
                  <button
                    onClick={executeWithdrawal}
                    style={{ padding: '8px 18px', borderRadius: 7, border: 'none', background: '#f59e0b', color: '#1f1100', cursor: 'pointer', fontWeight: 700, fontSize: 13 }}
                  >
                    Confirm Send
                  </button>
                </div>
      ...
      {walletView === 'explorer' && (
        <ExplorerView
          ...
          onLoad={async (off) => {
            ...
            setExplorerStatus('');
            try {
              const res = await window.wattcoinHardware.invoke('wattcoin-explorer-get-blocks', { offset: off, limit: EXPLORER_PAGE_SIZE });
              if (res && res.ok) {
      ...
      {walletView === 'staking' && (
        <StakingView
          ...
          queuedWtc={queuedSaleOrders.filter(o => o.status === 'queued' || o.status === 'delivery_pending').reduce((s, o) => s + (o.wtcAmount || 0), 0)}
        />
      )}
        ...
        <div style={{ marginTop: 4 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            ...
            <button
              ...
              style={{ background: transactionsBusy ? '#275a2f' : '#4ade80', color: transactionsBusy ? '#9fd4a8' : '#001008', border: 'none', borderRadius: 8, padding: '7px 12px', fontWeight: 700, cursor: transactionsBusy ? 'default' : 'pointer', fontSize: 12 }}
            >
          ...
          {transactionsStatus && (
            <div style={{ marginBottom: 10, fontSize: 13, color: transactionsStatus.startsWith('Failed') ? '#fca5a5' : '#9ac79f' }}>
              {transactionsStatus}
            </div>
          )}
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            {transactions.map((tx, idx) => {
              ...
              return (
                <div key={`${tx.txid || 'tx'}-${idx}`} style={{ background: '#0d1a0d', border: '1px solid #224022', borderRadius: 8, padding: '10px 12px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
                    <span style={{ fontSize: 12, color: outgoing ? '#fca5a5' : '#86efac', fontWeight: 700 }}>{outgoing ? 'Sent' : 'Received'}</span>
                    <span style={{ fontSize: 12, color: '#d7ffd9', fontWeight: 700 }}>{`${amount.toLocaleString(undefined, { maximumFractionDigits: 8 })} WTC`}</span>
                  </div>
                  <div style={{ fontSize: 11, color: '#9ac79f', marginBottom: 3 }}>Category: {tx.category || '-'}</div>
                  <div style={{ fontSize: 11, color: '#9ac79f', marginBottom: 3 }}>Confirmations: {Math.max(0, Number(tx.confirmations) || 0)}</div>
                  ...
                  {!!(networkInfo.explorerBaseUrl && tx.txid) && (
                    <button
                      onClick={() => {
                        if (window.wattcoinHardware && window.wattcoinHardware.invoke) {
                          window.wattcoinHardware.invoke('wattcoin-open-external-url', `${networkInfo.explorerBaseUrl}/tx/${tx.txid}`);
                        }
                      }}
                      style={{ display: 'inline-block', marginTop: 6, fontSize: 11, color: '#60a5fa', background: 'none', border: 'none', cursor: 'pointer', padding: 0, textDecoration: 'underline' }}
                    >
                      Open in explorer
                    </button>
                  )}
      ...
      {walletView === 'nfts' && (
        <div style={{ marginTop: 4 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button
                onClick={loadNfts}
                disabled={nftsBusy}
                style={{ background: nftsBusy ? '#275a2f' : '#4ade80', color: nftsBusy ? '#9fd4a8' : '#001008', border: 'none', borderRadius: 8, padding: '7px 12px', fontWeight: 700, cursor: nftsBusy ? 'default' : 'pointer', fontSize: 12 }}
              >
          ...
          {nftTransferMsg && (
            <div style={{ marginBottom: 10, fontSize: 13, color: /^(Failed|Error|Init failed|Init error|Transfer failed|Transfer error)/.test(nftTransferMsg) ? '#fca5a5' : '#86efac' }}>
              {nftTransferMsg}
            </div>
          )}

          {nfts.length === 0 && !nftsBusy && (
            <div style={{ fontSize: 14, color: '#9ac79f', textAlign: 'center', marginTop: 32 }}>No Wattcoin Vortex NFTs found for this address.</div>
          )}
            ...
            return (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 14 }}>
                {nfts.map(nft => {
                  ...
                  return (
                    <div key={nft.nftId} style={{ background: TIER_BG[tier] || '#0d1a0d', border: `1px solid ${TIER_COLORS[tier] || '#224022'}`, borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                      <img
                      ...
                      <div style={{ padding: '10px 12px', flex: 1, display: 'flex', flexDirection: 'column', gap: 4 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          ...
                          <span style={{ fontSize: 11, fontWeight: 700, color: TIER_COLORS[tier], background: `${TIER_COLORS[tier]}22`, borderRadius: 6, padding: '2px 7px' }}>{TIER_LABELS[tier] || tier}</span>
                        </div>
                        <div style={{ fontSize: 11, color: '#9ac79f' }}>{shares} profit {shares === 1 ? 'share' : 'shares'}</div>
                        <div style={{ fontSize: 10, color: '#6b8f6b', wordBreak: 'break-all' }}>ID: {nft.nftId}</div>
                        {!isTransferring ? (
                          <button
                            onClick={() => { setNftTransferTarget(nft.nftId); setNftTransferAddress(''); setNftTransferMsg(''); }}
                            style={{ marginTop: 6, background: '#1e3a1e', color: '#4ade80', border: '1px solid #4ade80', borderRadius: 8, padding: '6px 10px', fontWeight: 700, cursor: 'pointer', fontSize: 12 }}
                          >
                            Transfer
                          </button>
                        ) : (
                          <div style={{ marginTop: 6, display: 'flex', flexDirection: 'column', gap: 5 }}>
                            <input
                              ...
                              style={{ fontSize: 11, padding: '5px 8px', borderRadius: 6, border: '1px solid #224022', background: '#0d1a0d', color: '#d7ffd9', width: '100%', boxSizing: 'border-box' }}
                            />
                            <div style={{ display: 'flex', gap: 5 }}>
                              <button
                                onClick={() => {
                                  if (!nftTransferAddress.trim()) { setNftTransferMsg('Please enter a recipient address.'); return; }
                                  setNftTransferMsg('');
                                  setNftsBusy(true);
                                  window.wattcoinHardware.invoke('wattcoin-nft-transfer', {
                                    nftId: nft.nftId,
                                  ...
                                  }).catch(e => { setNftTransferMsg(`Error: ${e && e.message}`); })
                                    .finally(() => setNftsBusy(false));
                                }}
                                disabled={nftsBusy}
                                style={{ flex: 1, background: '#4ade80', color: '#001008', border: 'none', borderRadius: 6, padding: '6px 0', fontWeight: 700, cursor: nftsBusy ? 'default' : 'pointer', fontSize: 11 }}
                              >
                                Confirm
                              </button>
                              <button
                                onClick={() => { setNftTransferTarget(null); setNftTransferAddress(''); }}
                                style={{ flex: 1, background: '#1e3a1e', color: '#b7f5bc', border: 'none', borderRadius: 6, padding: '6px 0', fontWeight: 700, cursor: 'pointer', fontSize: 11 }}
                              >
                                Cancel
                              </button>
                            </div>
